...
class PackDeltaTests(unittest.TestCase):
    ...
    def test_compressed_hash_is_bound(self):
        data = blob()
        s = load_snapshot(self.write(data))
    ...
    def test_same_version_change(self):
        self.assertIn(
            ...
            self.rules(compare(self.snap(), self.snap({"package/a": b"a"}))),
        )
    ...
    def test_source_map_and_binary_and_bundle(self):
        b = self.snap(
            {
                "package/a.js.map": b"{}",
                "package/a.node": b"example",
                "package/node_modules/example/a": b"example",
            }
        )
        self.assertTrue(
            {"source-map", "binary-payload", "bundled-dependency"}
            <= self.rules(compare(self.snap(), b))
        )

    def test_executable_mode(self):
        item = tarfile.TarInfo("package/run.sh")
        item.mode = 0o755
        item.size = 4
    ...
    def test_links_and_special_types(self):
        for kind in (tarfile.SYMTYPE, tarfile.LNKTYPE, tarfile.CHRTYPE, tarfile.FIFOTYPE):
            item = tarfile.TarInfo("package/special")
            item.type = kind
            item.linkname = "example"
            with self.subTest(kind=kind), self.assertRaises(InputError):
                self.snap(members=[(item, b"")])

    def test_directory_is_allowed(self):
        item = tarfile.TarInfo("package/docs/")
    ...
    def test_missing_manifest(self):
        ...
        with tarfile.open(fileobj=out, mode="w") as archive:
            item = tarfile.TarInfo("package/readme")
    ...
    def test_all_size_limits(self):
        cases = [
            ...
            Limits(expanded_bytes=1024),
            Limits(file_bytes=1),
            Limits(total_file_bytes=1),
            Limits(manifest_bytes=1),
            Limits(members=0),
        ]
        for limits in cases:
    ...
    def test_deterministic_report(self):
        a = self.snap()
        b = self.snap({"package/z": b"z", "package/a": b"a"})
    ...
    def test_cli_findings_and_never_exit_codes(self):
        a = self.write(blob())
        b = self.write(blob({"package/new.map": b"{}"}))
    ...
    def test_cli_error_is_value_free_even_with_never(self):
        ...
        with contextlib.redirect_stderr(output):
            result = main(
                [str(self.root / "private-example-input"), "also-missing", "--fail-on", "never"]
            )
    ...
    def test_real_cli_process(self):
        a = self.write(blob())
        b = self.write(blob({"package/a.map": b"{}"}))
        proc = subprocess.run(
            [sys.executable, "-m", "packdelta", str(a), str(b), "--format", "json"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            timeout=20,
    ...
    def test_zipapp_reproducible_and_portable(self):
        def run():
            return subprocess.run(
                [sys.executable, "tools/build_zipapp.py"],
                cwd=ROOT,
                capture_output=True,
                text=True,
                check=True,
        ...
        run()
        first = (ROOT / "dist/packdelta.pyz").read_bytes()
        run()
        self.assertEqual(first, (ROOT / "dist/packdelta.pyz").read_bytes())
        portable = self.root / "packdelta.pyz"
        portable.write_bytes(first)
        a = self.write(blob())
        proc = subprocess.run(
            [sys.executable, str(portable), str(a), str(a), "--format", "json"],
            cwd=self.root,
            capture_output=True,
            text=True,
            timeout=20,
